    ...
    public function home(Request $req)
{
    ...
    $atividades = Atividade::select('atividades.*')
        ...
        ->where('atividades.company_id', $company_id) // Adicionando filtro de company_id
        ->limit(10)
        ->orderBy('id', 'desc')
        ->get();
    ...
    foreach($atividades como $row_atividade) {
        $atividade_funcionario = AtividadeFuncionario::where('atividade_id', $row_atividade->id)
            ->where('company_id', $company_id) // Adicionando filtro de company_id
            ->value('id');
        ...
        $row_atividade->observacao = Observacao::select('message')
            ->where('id', $senderId)
            ->where('company_id', $company_id) // Adicionando filtro de company_id
            ->value('message');
        ...
        $funcionario_id = AtividadeFuncionario::where('atividade_id', $row_atividade->id)
            ->where('company_id', $company_id) // Adicionando filtro de company_id
            ->value('funcionario_id');
        $row_atividade->funcao_title = Funcao::where('id', $row_atividade->funcao_id)
            ->where('company_id', $company_id) // Adicionando filtro de company_id
            ->value('title');
        $row_atividade->funcionario_id = $funcionario_id;
        $row_atividade->user_id = User::select('users.id')
            ->join('funcionarios', 'users.id', 'funcionarios.user_id')
            ->where('funcionarios.id', $funcionario_id)
            ->where('funcionarios.company_id', $company_id) // Adicionando filtro de company_id
            ->value('users.id');
        $row_atividade->user_name = User::select('name')
            ->join('funcionarios', 'users.id', 'funcionarios.user_id')
            ->where('funcionarios.id', $funcionario_id)
            ->where('funcionarios.company_id', $company_id) // Adicionando filtro de company_id
            ->value('name');
    ...
    $funcoes_list = Funcao::where('company_id', $company_id)
        ->where('status', 1)
        ->limit(10)
        ->orderBy('id', 'DESC')
        ->get();
    ...
    $jornadas = Jornada::where('company_id', $company_id)
        ->where('status', 1)
        ->limit(10)
        ->orderBy('id', 'DESC')
        ->get();
    ...
    $funcionarios_list = Funcionario::where('company_id', $company_id)
        ->limit(10)
        ->orderBy('id', 'DESC')
        ->get();
